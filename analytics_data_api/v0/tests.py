...
from analytics_data_api.v0.models import CourseEnrollmentByBirthYear, CourseEnrollmentByEducation, EducationLevel, \
    CourseEnrollmentByGender, CourseActivityByWeek
...
class CourseActivityLastWeekTest(TestCaseWithAuthentication):
    def setUp(self):
        ...
        G(CourseActivityByWeek, course_id=self.course_id, interval_start=interval_start, interval_end=interval_end,
          label="posted_forum", count=100)
        G(CourseActivityByWeek, course_id=self.course_id, interval_start=interval_start, interval_end=interval_end,
          label="attempted_problem", count=200)
        G(CourseActivityByWeek, course_id=self.course_id, interval_start=interval_start, interval_end=interval_end,
          label="active", count=300)
        G(CourseActivityByWeek, course_id=self.course_id, interval_start=interval_start, interval_end=interval_end,
          label="played_video", count=400)
...
class CourseEnrollmentViewTestCase(object):
    ...
    def _get_non_existent_pk(self):
        pk = random.randint(100, 9999)

        if not self.model.objects.filter(course_id=pk).exists():
            return pk

        return self._get_non_existent_pk()

    def test_get_not_found(self):
        """
        Requests made against non-existent courses should return a 404
        """
        course_id = self._get_non_existent_pk()
        self.assertFalse(self.model.objects.filter(course_id=course_id).exists())   # pylint: disable=no-member
...
class CourseEnrollmentByGenderViewTests(TestCaseWithAuthentication, CourseEnrollmentViewTestCase):
    ...
    def setUpClass(cls):
        cls.course_id = 1
        cls.ce1 = G(CourseEnrollmentByGender, course_id=cls.course_id, gender='m')
        cls.ce2 = G(CourseEnrollmentByGender, course_id=cls.course_id, gender='f')
